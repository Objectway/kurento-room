...
import {INotificationParticipantJoined} from "./KASConnection";
import {INotificationParticipantLeft} from "./KASConnection";
import {INotificationParticipantEvicted} from "./KASConnection";
import {INotificationParticipantPublished} from "./KASConnection";
import {INotificationReceiveICECandidate} from "./KASConnection";
import {INotificationParticipantUnpublished} from "./KASConnection";
import {INotificationMediaError} from "./KASConnection";
...
import {IRequestSendMessage} from "KASServerAPI";
...
export class KASServer {
    ...
    /**
     * ICE options (used when subscribing to remote streams)
     */
     ...
     * @type {undefined}
     */
    private onParticipantLeftCallback: (params: INotificationParticipantLeft) => any = undefined;

    /**
     * (Optional) callback invoked when a new message is received.
     * @type {undefined}
     */
    private onParticipantEvictedCallback: (params: INotificationParticipantEvicted) => any = undefined;
     ...
     */
    public joinRoom = (roomId: string, username: string, thenCallback?: (room: KASRoom, response: IResponseJoinRoom) => any, catchCallback?: (error: Object) => any): void => {
        // TODO: For now, there can be only one room per connection
    ...
    /**
     * Sends a request to the server, calling the appropriate callbacks.
     *
     * @param method
     * @param message
     * @param username
     ...
     * @param catchCallback
     */
    public sendMessage = (message: string, username: string, roomId: string, thenCallback?: () => any, catchCallback?: (error: Object) => any): void => {
        const request: IRequestSendMessage = <IRequestSendMessage> {
            "message": message,
            "userMessage": username,
            "roomMessage": roomId
        };

        this.serverAPI.sendMessage(request, thenCallback, catchCallback);
    };

    /**
     * Sends a custom request to the server, calling the appropriate callbacks.
     *
     * @param method
     * @param message
     * @param username
     ...
     * @param catchCallback
     */
    public sendCustomRequest = (customRequest: Object, thenCallback?: (response: any) => any, catchCallback?: (error: Object) => any): void => {
        this.serverAPI.sendCustomRequest(customRequest, thenCallback, catchCallback);
    };

    /**
     * Disposes the rooms
     */
    public dispose = (forced: boolean, thenCallback?: () => any, catchCallback?: (error: Object) => any): void => {
        // forced means the user was evicted, no need to send the 'leaveRoom' request
        if (!forced) {
            // TODO: For now, there can be only one room per connection
            // Leave each room
            this.leaveRoom(this.getOnlyRoom().getId(), this.getOnlyParticipantId(), thenCallback, catchCallback);
        } else {
            if (thenCallback !== undefined) {
                thenCallback();
            }
    ...
    private onParticipantLeft = (params: INotificationParticipantLeft): void => {
        ...
        // Invoke the callback before we actually remove it from our internal data structures
        // (in case they are needed by the callback)
        if (this.onParticipantLeftCallback !== undefined) {
            this.onParticipantLeftCallback(params);
        }
    ...
    private onParticipantEvicted = (params: INotificationParticipantEvicted): void => {
        ...
        // Invoke the callback before we actually remove it from our internal data structures
        // (in case they are needed by the callback)
        if (this.onParticipantEvictedCallback !== undefined) {
            this.onParticipantEvictedCallback(params);
        }
    ...
    public setOnParticipantLeftCallback = (onParticipantLeftCallback: (params: INotificationParticipantLeft) => any): void => {
        this.onParticipantLeftCallback = onParticipantLeftCallback;
    };

    public setOnParticipantEvictedCallback = (onParticipantEvicted: (params: INotificationParticipantEvicted) => any): void => {
        this.onParticipantEvictedCallback = onParticipantEvicted;
    };
