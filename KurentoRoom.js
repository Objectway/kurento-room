...
function Room(kurento, options) {
  ...
  this.recvIceCandidate = function (msg) {
    ...
    // In our modified version of Kurento-Room, each endpoint has appended its streamId.
    // For example: Cartier.A3-webcam, Cartier.A3-screen, ...
    var idx = msg.endpointName.lastIndexOf('-');
    if (idx == -1) {
      console.error("Participant endpoint name " + msg.endpointName + " does not contain the streamId part. Ice candidate will be ignored.", candidate);
      return false;
    }

    // Correct the endpoint name
    var endpointName = msg.endpointName.substring(0, idx);
    var streamId = msg.endpointName.substring(idx + 1);
    msg.endpointName = endpointName;
  ...
  this.onReconnected = function() {
    ...
    var room = that.name;
    if (room !== undefined) {
      ee.emitEvent('socket-reconnected', [{
        room: room
      }]);
    } else {
...
function Stream(kurento, local, room, options) {
  ...
  this.init = function () {
    ...
    // Get the constraints from the options object
    ...
    if(options.data && !options.video && !options.audio) {
      ee.emitEvent('datachannel-created', null);
    } else {
      getUserMedia(constraints, function (userStream) {
        wrStream = userStream;
        ee.emitEvent('access-accepted', null);
