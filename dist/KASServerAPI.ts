...
export class KASServerAPI {
    ...
    /**
     * Sends a customRequest request, calling the appropriate callbacks.
     * @param customRequest
     * @param thenCallback
     * @param catchCallback
     */
    public sendCustomRequest = (customRequest: Object,
                                thenCallback?: (response: any) => any,
                                catchCallback?: (error: any) => any): void => {
        this.kasConnection.sendRequest('customRequest', customRequest, (error: any, response: IResponseSendMessage) => {
            if (error) {
                if (catchCallback !== undefined) {
                    catchCallback(error);
                }
